/**
 * Broker-data content for the /trades page.
 *
 * This is an async server component rendered behind a <Suspense> boundary by
 * `trades/page.tsx`, so the page SHELL (GrShell nav, hero title, account picker)
 * paints instantly while this — which performs the slow broker work (ABH day
 * net + the historical Fills report) — streams in.
 *
 * The broker-loading logic and the KPI/filter/table markup are RELOCATED here
 * verbatim from the page: ABH remains the source of truth for day net / KPI net
 * and the two-tier derived-fee model (PR #170) is unchanged. The only thing that
 * changed is WHERE this runs — off the page's blocking render path.
 */
...
// Non-critical broker enrichment caps (ABH itself is NOT capped — source of truth).
...
type Props = {
  ...
  tz: string;
  activeAccounts: CommandCenterAccount[];
  selectedAccountIsExpired: boolean;
};

export async function TradesContent({
  selectedAccount,
  userId,
  filter,
  rangeDays,
  ...
  // The lookback window normally tracks the range toggle. A date deep-link may
  // point at a day OLDER than 31 days, so when a date filter is active we extend
  // `since` to cover that exact date (minus a small buffer for entries that
  // opened on a prior day). Then we narrow to the day client-side (tz-safe).
  ...
  const dbFillInputs = await loadAccountFillInputs(selectedAccount.id, { since });

  // Broker-authoritative day-level realized P&L (ABH, source of truth) + the
  // historical Fills report (table rows for trades closed before Guardrail
  // connected). Read-only, best-effort: any failure yields {} and day totals
  // fall back to fill values.
  ...
  {
    const client = new TradovateClient(selectedAccount.id, userId);
    try {
      ...
      // NAME is required as the report's `account` param. The Fills report
      // endpoint returns 0 rows for large windows, so the window is narrowed to
      // what the page displays: ±1 day around dateFilter, or the visible range.
      try {
        ...
        if (accountName) {
          ...
          if (rows === null) {
            ...
            console.info("[trades/page] fills-report timed-out or null", {
              accountId: selectedAccount.id, accountName, dateFilter, rangeDays, startStr, endStr,
            });
          } else {
            const inWindow = historicalFillsToFillInputs(rows).filter((f) => f.occurredAt >= since);
            historicalFillInputs = inWindow;
            usedReportFills = inWindow.length > 0;
            console.info("[trades/page] fills-report", {
              accountId: selectedAccount.id, accountName, dateFilter, rangeDays, startStr, endStr,
              reportRowsCount: rows.length, mergedFillInputsCount: inWindow.length,
            });
          }
        }
      } catch {
      ...
      // init failed — broker data unavailable, render DB fills only
    }
  }
  ...
  console.info("[trades/page] reconstructed", {
    accountId: selectedAccount.id, dateFilter, rangeDays,
  ...
  // across their trades. Build one id → {fees, net, feeSource} map.
  ...
  // Broker-net totals for the KPI primary: sum ALL broker days in the window.
  ...
  return (
    <>
      {/* ── Broker provenance subtitle (ABH-dependent, streamed) ─────────── */}
      {(brokerSource !== "none" || earliestTradeDate != null || lowConfidence) && (
        <section style={{ padding: "0 36px 8px" }}>
          {!dateFilter && (brokerSource !== "none" ? earliestBrokerDay != null : earliestTradeDate != null) && (
            <div style={{ fontSize: 11.5, color: "var(--gr-text-mute)" }}>
      ...
      {selectedAccountIsExpired && (
        <section style={{ padding: "0 36px 16px" }}>
          ...
          >
            ...
            <Link
              href="/dashboard"
              style={{ fontSize: 11.5, color: "var(--gr-copper)", textDecoration: "none", flexShrink: 0 }}
            >
              Manage on dashboard →
            </Link>
          </div>
      ...
      <section style={{ padding: "0 36px 18px" }}>
        <div className="trades-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          {[
            ...
            //   1. Broker day-net — authoritative after-fees total.
            ...
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
              ...
              <tbody>
                {groupedDateKeys.map((dateKey) => {
                  ...
                  // Day total truth order: broker net (after fees) → per-trade net
                  // (only if every trade has fees) → fill P&L before fees.
                      ...
                      {rows.map((t) => {
                        ...
                        return (
                          <tr key={t.id} style={{ borderBottom: "1px solid var(--gr-border-sub)" }}>
                            ...
                            {/* Fees — exact or Account-Balance-derived; "Not reported" only when neither. */}
...
/** Skeleton shown while TradesContent streams in — keeps the KPI grid + table
 *  footprint so navigation feels instant and layout shift is minimal. */
export function TradesContentLoading() {
  const cardStyle = {
    background: "var(--gr-surface)",
    border: "1px solid var(--gr-border)",
    borderRadius: 12,
  ...
  return (
    <>
        ...
        <div className="trades-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} style={cardStyle}>
              ...
              <div style={{ fontSize: 22, fontWeight: 600, marginTop: 6, lineHeight: 1, color: "var(--gr-text-faint)", fontFamily: "var(--font-ibm-plex-mono, monospace)" }}>…</div>
              <span style={{ fontSize: 11, marginTop: 6, display: "inline-block", color: "var(--gr-text-mute)" }}>
                {i === 0 ? "Loading broker performance…" : ""}
              </span>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "0 36px 36px" }}>
        <div style={{ background: "var(--gr-surface)", border: "1px solid var(--gr-border)", borderRadius: 14, padding: "48px 24px", textAlign: "center", minHeight: 160 }}>
          <span style={{ fontSize: 12.5, color: "var(--gr-text-mute)" }}>Loading trades…</span>
        </div>
      </section>
    </>
  );
