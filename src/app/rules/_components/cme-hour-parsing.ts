...
export function cmeHourBoundaryNote(hour: number): string | null {
  if (hour === 16) {
    return "4:00 PM CT is the CME daily break Monday–Thursday and weekly close Friday. Automatic cutoff enforcement is not active yet.";
  }
  if (hour === 17) {
    return "5:00 PM CT is the CME session reopen Sunday–Thursday. Trades scheduled before reopen may not execute.";
  }
  return null;
}
