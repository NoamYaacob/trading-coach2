  ...
  return (
    ...
    >
      <div className="grid gap-8 sm:gap-12">
        ...
        {/* ── Protection path ─────────────────────────────────────────── */}
        <section
          className="rounded-[14px] border border-amber-200/60 bg-amber-50/20 p-5 sm:p-8"
        >
          ...
          <p className="mt-4 text-[11px]" style={{ color: "var(--gr-text-mute)" }}>
            * Partial or coming-soon rules. See{" "}
            <a href="/features" className="underline transition hover:opacity-70">
              features
            </a>{" "}
            for current status.
          </p>
        ...
        <section className="rounded-[14px] border border-amber-200/80 bg-amber-50/30 p-5 sm:p-8">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {PROP_FIRM_CARDS.map((card) => (
              <div
                key={card.title}
                className="rounded-2xl border border-amber-100 px-5 py-5"
                style={{ background: "var(--gr-surface)" }}
              >
                <p className="text-sm font-semibold" style={{ color: "var(--gr-ink)" }}>{card.title}</p>
                <p className="mt-2 text-sm leading-6" style={{ color: "var(--gr-text-mid)" }}>{card.body}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        >
          ...
          <p className="mt-3 text-sm leading-6" style={{ color: "var(--gr-text-mid)" }}>
        ...
        >
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {[
              ...
              >
                <p className="text-sm font-semibold" style={{ color: "var(--gr-ink)" }}>{item.label}</p>
                <p className="mt-2 text-sm leading-5" style={{ color: "var(--gr-text-mid)" }}>{item.text}</p>
              </div>
            ))}
          </div>
...
function StageCard({
  ...
  return (
    <div
      className="flex-1 rounded-2xl border border-amber-100 px-4 py-4"
      style={{ background: "var(--gr-surface)" }}
    >
      <div className="mb-2">
      ...
      <p className="mt-1 text-[11px] leading-4" style={{ color: "var(--gr-text-mute)" }}>{risk}</p>
      <div className="mt-3 rounded-xl bg-amber-50 px-3 py-2.5">
        ...
        <ul className="grid gap-1">
          {rules.map((r) => (
            <li key={r} className="flex items-center gap-1.5 text-[10px]" style={{ color: "var(--gr-text-mid)" }}>
