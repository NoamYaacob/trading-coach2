...
const INTENT_CONTEXT: Record<DistressIntent, { situation: string; goal: string }> = {
  stop_fomo: {
    ...
    goal: "One or two lines: the pull, then the redirect. Aim for this register: 'הסטאפ לא היה שם. הבא יבוא.' or 'לפספס מכאיב. לרדוף אחריו — עוד יותר.' — adapt to the moment, don't copy.",
  },
  stop_revenge: {
...
export function buildHebrewDistressPrompt(
  ...
  lines.push(
    ...
    '  Dragged: "הכרת בזה. זה כבר הצעד הראשון."',
    "",
