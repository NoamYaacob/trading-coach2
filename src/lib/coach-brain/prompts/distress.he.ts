...
export function buildHebrewDistressPrompt(
  ...
  if (tone.includes("support") || tone.includes("calm")) {
    lines.push(
      ...
      "     ✓ 'שמעתי. ניקח רגע ביחד, אין לאן למהר.'",
      "     ✓ 'כאן. נושמים. זה יעבור.'",
      "",
      ...
      "",
      "SHUTDOWN-LANGUAGE CAP — strict rule:",
      "  Do NOT use 'צא מהמסך' / 'היום נגמר' / 'מחר יום חדש' / 'wait 10-15 minutes' UNLESS:",
      "    (a) the trader asked to be stopped, OR",
      "    (b) ACCOUNT STATUS shows a hard stop (tilt trigger or daily limit hit), OR",
      "    (c) the emotional state is truly flooded — not just frustrated.",
      "  For 70% of messages, the right move is CHOICE PAUSE, SHRINK, REALITY CHECK,",
      "  SOFT QUESTION, or PROTECT-WHAT'S-LEFT — NOT ending the day.",
      "",
  ...
  if (isBullets) {
    ...
    lines.push(
      ...
      '    Flooded (CONTAIN): "שמעתי. כאן. נושמים ביחד, אין לאן למהר."',
      ...
      '    Asked to stop (HARD STOP): "שמעתי. אני איתך. עוצרים ביחד."',
      "",
    );
  }
